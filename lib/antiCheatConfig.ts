...
function parsePositiveIntegerEnv(rawValue: string | undefined, defaultValue: number): number {
  if (!rawValue) {
    return defaultValue
  }
...
export function getAntiCheatConfig(): AntiCheatConfig {
  return {
    enabled: parseBooleanEnv(process.env.ANTI_CHEAT_ENABLED, DEFAULT_ANTI_CHEAT_ENABLED),
    minGameDurationMs: parsePositiveIntegerEnv(
      process.env.ANTI_CHEAT_MIN_GAME_DURATION_MS,
      DEFAULT_MIN_GAME_DURATION_MS,
    ),
    sessionTtlMs: parsePositiveIntegerEnv(process.env.ANTI_CHEAT_SESSION_TTL_MS, DEFAULT_SESSION_TTL_MS),
  }
}
