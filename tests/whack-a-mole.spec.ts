...
async function getStats(
  page: Page,
): Promise<{ hits: number; misses: number; accuracy: number }> {
...
test.describe("Whack-a-Mole game outcomes", () => {
  ...
  test("direct /game visit without a stored name redirects to landing page", async ({
    page,
  }) => {
  ...
  test("no interaction → game finishes on its own with 0 hits and 0 misses", async ({
    page,
  }) => {
    await enterNameAndStart(page);
    await page.getByRole("button", { name: /start game/i }).click();
